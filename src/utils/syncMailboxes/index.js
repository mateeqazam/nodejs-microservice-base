...
import enqueueMailboxForSync from './enqueueMailboxForSync';
...
async function processMailboxes(mailboxes) {
	try {
		...
		await Promise.all(map(mailboxes, (mailbox) => pLimit(() => enqueueMailboxForSync(mailbox))));
		return {};
	} catch (error) {
...
async function syncMailboxes() {
	try {
		...
		const errorMessage = `[syncMailboxes] Exception: ${error?.message}`;
		log.error(errorMessage, { error });
		return { error };
	}
}

export default syncMailboxes;
